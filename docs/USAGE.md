...
## 12. 编排 Run（Claude 规划/审查 ↔ Codex 执行，自动循环）

App 顶栏切到 **「编排 Run」**：输入一个高层目标 →

1. **Claude Code 规划**：拆成多个阶段，每阶段含 代码规划 / UI 规划 / 验收标准。
2. **Codex 逐阶段执行**。
3. **Claude 审查**：按该阶段验收标准审 diff → 通过进入下一阶段；不通过把意见回灌 Codex **修订**（至多 N 次）。
4. 循环至全部阶段完成。全程你只看，可随时暂停 / 介入 / 中止。

- **闸门**：默认「全自动」（Claude 审查即闸门）；也可选「计划需确认」「每阶段需确认」。
- **介入**：运行中注入一条指令，下次执行/修订带上。
- **待人工**：修订耗尽或执行器报错 → 标记「待人工」并暂停，你看完意见点「继续」。
- 每阶段可展开看 代码/UI 规划、验收标准、审查结论、以及该阶段 diff。
- **前提**：`claude` 与 `codex` 都已登录（规划/审查用 claude，执行用 codex）。Run 状态跨重启可恢复（重启后显示「已暂停」，点「继续」续跑）。
